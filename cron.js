...
//   "FOE.OL", "FRO.OL", "FUNCOM.OL", "GJF.OL", "GOGL.OL", "GOD.OL", "GSF.OL", "HFISK.OL", "HAVI.OL", "HEX.OL", "HBC.OL", "HLNG.OL", "IDEX.OL",
    ...
    request(url, function (error, response, body) {
      ...
      if (!error && response.statusCode == 200) {
        ...
        if (!d) {
          ...
          if (rsi_yesterday < 60 && rsi_today > 60) {
            // console.log(ticker = ': MACD Crossed over 0 today!');
            writeToFile(ticker + ': RSI Crossed over 60 today!')
          } else if (rsi_yesterday > 60 && rsi_today < 60) {
            // console.log(ticker + ': MACD Crossed under 0 today!');
            writeToFile(ticker + ': RSI Crossed under 60 today!')
          } else {
            // writeToFile(ticker + ': No results!')
          }
        }
